...
####### load model #######
cfg_file = "./cfgs/models/kitti/VirConv-T.yaml"
...
ckpt = "../output/models/kitti/VirConv-T/default/ckpt/checkpoint_epoch_50.pth"
...
####### read input #######
batch_dict = torch.load("../ckpt/VirConv-T2.pth", map_location="cuda")
inputs = batch_dict
####### read input #######

####### DSVT #######
class VirConvTRT(nn.Module):
...
####### torch to onnx #######
with torch.no_grad():
    VirConv_Backbone = model.backbone_3d
    dsvtblocks_list = VirConv_Backbone.stage_0
    layer_norms_list = VirConv_Backbone.residual_norm_stage_0
    inputs = model.vfe(inputs)
    voxel_info = VirConv_Backbone.input_layer(inputs)
    ...
    allptransblocktrt = VirConvTRT(dsvtblocks_list, layer_norms_list).eval().cuda()
    torch.onnx.export(
        allptransblocktrt,
        alldsvtblockstrt_inputs,
